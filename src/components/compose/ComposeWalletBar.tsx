...
export default function ComposeWalletBar({ onConnected }: { onConnected: () => void }) {
  const wallet = useComposeStore((s) => s.wallet);
  const setWallet = useComposeStore((s) => s.setWallet);
  ...
  async function handleConnect(provider: WalletProvider) {
    ...
    try {
      ...
      if (state.taprootAddress === state.paymentAddress) {
        throw new Error('Compose requires both a taproot and a payment address.');
      }
      setWallet(state);
      onConnected();
    } catch (err) {
